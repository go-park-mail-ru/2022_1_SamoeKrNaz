...
func CreateCheckListItemMock() (repositories.CheckListItemRepository, sqlmock.Sqlmock, error) {
	...
	repoListItem := MakeCheckListItemRepository(openGorm)
	return repoListItem, mock, err
}

func TestSelectByIdCheckListItem(t *testing.T) {
	t.Parallel()
	...
	expect := []*models.CheckListItem{
		{IdClIt: elemID, Description: "description", IdCl: 1, IdT: 1},
	}
	...
	item, err := repoCheckListItem.GetById(elemID)
	if err != nil {
		t.Errorf("unexpected err: %s", err)
		return
	}
	...
	_, err = repoCheckListItem.GetById(2)
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("there were unfulfilled expectations: %s", err)
		return
	}
